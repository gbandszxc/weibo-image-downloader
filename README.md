...
在 Tampermonkey/Violentmonkey 的脚本菜单中点击 **“视频下载：关闭/开启”** 可切换视频下载。默认关闭，关闭时脚本行为与图片下载版本一致；开启后请刷新页面，让当前页面重新解析媒体项。
...
### 未发布
- 新增：油猴菜单可切换视频下载，默认关闭以保持原图片下载行为
- 新增：开启视频下载后，微博视频按最高可用清晰度下载，长按选择弹窗使用蓝色系 `v1`、`v2` 标识视频
- 新增：开启视频下载后，X 页面中可直接解析到直链的普通视频会纳入下载
- 保持：微博 Live Photo、微博 GIF、X GIF 的现有下载规则不变
