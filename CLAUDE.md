...
6. 适配新站点、新域名或新入口页面时，务必同步检查并补全 userscript metadata 中的 `@match` 规则，避免脚本未在目标页面生效。
7. 发布前务必运行 `bun run verify:dist`，确认 `dist/weibo-image-downloader.user.js` 与当前源码构建结果一致，避免提交过时产物。
