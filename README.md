...
- X 纯视频帖子不显示下载按钮；同条内容里若仍有图片/GIF，则保留图片/GIF 下载按钮
- 批量下载支持，避免浏览器并发限制
...
5. X 动图（GIF）会下载为 `.mp4` 动图源；纯视频帖子不会显示图片下载按钮
...
### v1.4.2
- 修复：微博详情页部分帖子在 `GM_download` 失败后不再退化为批量打开图片新标签页，改为使用二进制兜底下载
- 修复：Tampermonkey `@connect` 改为根域名声明，兼容微博图片子域名下载
- 修复：补充 `@connect *` 兜底，兼容微博图片下载中的重定向与未预料域名校验
- 修复：微博图片二进制兜底请求补充微博来源头，避免图片服务返回 403
- 优化：下载失败时会显示明确失败提示，避免误报“已下载”
